...
export default function MenuGridClient({ items, activeCategory, cart, setCart, categories }: any) {
  ...
  const grouped = useMemo(() => {
    ...
    for (const it of filtered) {
      const catId = it.category_id || 'uncat'
    ...
    const catIdsInOrder = (categories ?? []).map((c: any) => c.id)
